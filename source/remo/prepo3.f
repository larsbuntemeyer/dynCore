C
C
C**** PREPBOD  -   UP:AUFBEREITUNG DER MONATSWEISE VORHANDENEN OZONFELDER
C**
C**   AUFRUF   :   CALL PREPO3 IN UP *EC4ORG*
C**
C**   ENTRIES  :   KEINE
C**
C**   ZWECK    :   AUFBEREITUNG DER MONATSWEISE VORHANDENEN OZONFELDER
C**                OZONPL
C**
C**   VERSIONS-
C**   DATUM    :   15.03.06
...
C
      SUBROUTINE PREPO3(OZONPL, ZOZACT, NOZ)
C
      IMPLICIT NONE
C
      INCLUDE "parorg.h"
...
C
      ...
      REAL,    INTENT(IN)    :: ZOZACT(IEJE,NOZ,14)
      REAL,    INTENT(INOUT) :: OZONPL(IEJE,NOZ)
C
C     Local Variables
C
     ...
     &           IMP1, IMP1SEC, IMSECA, IMSECB, ISCHLT, ISEC, IY, K, M
      ...
      INTEGER :: MONDAY(14), MONATC(14)
C
      DATA  MONATC / 31, 31, 28, 31, 30, 31, 30 ,
...
C
         ...
         DO K=1,NOZ
            DO IJ=1,IEJE
               OZONPL(IJ,K)=ZW1*ZOZACT(IJ,K,IM)+ZW2*ZOZACT(IJ,K,IMM1)
            ENDDO
         ENDDO
C
...
C
         ...
         DO K=1,NOZ
            DO IJ=1,IEJE
               OZONPL(IJ,K)=ZW1*ZOZACT(IJ,K,IM)+ZW2*ZOZACT(IJ,K,IMP1)
            ENDDO
         ENDDO
C
      ENDIF
