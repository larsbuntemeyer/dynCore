      SUBROUTINE AOSWR ( KLON, KLEV, KEWAER, KAERH, KNU
     &  , PPMB
...
C    &  , PAER, PALBS, PCG, PCLDSW, PDSIG, POMEGA, PRAYL, PSEC, PTAU
C    &  , PCGAZ, PPIZAZ, PRAY1, PRAY2, PREFZ, PRJ, PRK, PRMUE
C    &  , PTAUAZ, PTRA1, PTRA2 )
C
...
C     Local Variables
C
      INTEGER :: IAE, ICAE, IH, JA, JAE, JAER, JAJ, JK, JKL, JKLP1,
     &           JL, JKM1
      REAL    :: 
     &     ZBMU0, ZBMU1, ZCLTEST, ZCORAE, ZCORCD, ZDEN, ZDEN1, ZEODAE,
     &     ZFACOA, ZFACOC, ZFF, ZGAP, ZGAR, ZMU1, ZMUE, ZR21, ZR22,
     &     ZRATIO, ZRE11, ZRHODZ
      REAL    :: ZSS1, ZTO, ZTRAY, ZWW
C     
...
C
C
C*    0.9 THRESHOLD FOR ZERO AEROSOL OPTICAL DEPTH
C         ----------------------------------------
      ZEODAE=1.E-15
C
C     ------------------------------------------------------------------
...
C     CONTRIBUTION GADS AEROSOLS
         DO JAER=1,KEWAER
            ...
            IAE=JAER+5
            DO JL=1,KLON
               IH=KAERH(JL,KLEV+1-JK)
               ZRHODZ=10.*(PPMB(JL,JK)-PPMB(JL,JK+1))/G
               PTAUAZ(JL,JK)=PTAUAZ(JL,JK)
     &              +PAER(JL,JK,IAE)*TAUAN(IH,KNU,ICAE)*FCVAER(ICAE)
     &              *ZRHODZ
               PPIZAZ(JL,JK)=PPIZAZ(JL,JK)+PAER(JL,JK,IAE)
     &              *TAUAN(IH,KNU,ICAE)*PIZAN(IH,KNU,ICAE)*FCVAER(ICAE)
     &              *ZRHODZ
               PCGAZ (JL,JK)=PCGAZ (JL,JK)+PAER(JL,JK,IAE)
     &              *TAUAN(IH,KNU,ICAE)*PIZAN(IH,KNU,ICAE)
     &              *CGAN(IH,KNU,ICAE)*FCVAER(ICAE)*ZRHODZ
            ENDDO
         ENDDO
C
         DO JL = 1 , KLON
            IF(PTAUAZ(JL,JK).GT.ZEODAE) THEN
